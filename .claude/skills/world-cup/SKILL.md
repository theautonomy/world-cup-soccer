...
user_invocable: true
args: mode
argument-hint: "team {name} | refine {name} | audit | audit {name} | match {team1} vs {team2} | player {name} | group {A-L}"
---
...
| `team {name}` | `team` |
| `refine {name}` | `refine` — refresh existing JSON with latest info |
...
可用指令：
  ...
  /world-cup refine {球队名}            → 刷新现有数据（最新阵容、排名、教练动态）
