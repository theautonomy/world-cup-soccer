...
export default function Hero({ team, worldCup }) {
  const { lang } = useApp()
  const poem = worldCup?.poem_zh

  return (
    <section className={styles.hero}>
      <div className={`${styles.inner} ${lang === 'zh' && poem ? styles.hasPoem : ''}`}>
        ...
        {lang === 'zh' && poem && (
          <div className={styles.poem}>
            {poem.split('\n').map((line, i) => (
              <div key={i} className={styles.poemLine}>{line}</div>
            ))}
          </div>
        )}
      </div>
